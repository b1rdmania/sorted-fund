...
async function main() {
  ...
  // Get contracts
  const testCounter = new ethers.Contract(
    TEST_COUNTER,
    ['function getCounter(address) view returns (uint256)', 'function increment()'],
    provider
  );
  const simpleAccount = new ethers.Contract(
    SIMPLE_ACCOUNT,
    ['function getNonce() view returns (uint256)', 'function execute(address,uint256,bytes)'],
    provider
  );
  ...
  const userBalanceBefore = await provider.getBalance(SIMPLE_ACCOUNT);
  ...
  const userBalanceAfter = await provider.getBalance(SIMPLE_ACCOUNT);
